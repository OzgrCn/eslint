            ...
            if (node.init && node.id) {
                if ((node.init.type === "BinaryExpression")) {
                    if (node.init.left.type === "Identifier") {
                        identifierName = node.init.left.name;
                    } else if (node.init.right.type === "Identifier") {
                        identifierName = node.init.right.name;
                    }
                } else {
                    identifierName = node.init.name;
                }
                if (identifierName === node.id.name) {
                    context.report(node.id, "\"{{a}}\" was used before it was defined", {a: node.id.name});
                }
            }
        }
    };
